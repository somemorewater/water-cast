...
goLiveBtn.addEventListener("click", () => {
  ...
  document
    .querySelectorAll(
      "#streamForm input, #streamForm select, #streamForm textarea",
    )
    .forEach((input) => {
      input.disabled = true;
    });
...
// Test Camera Button - FIXED: Added error handling
...
// Copy Stream Key - FIXED: Using modern Clipboard API instead of deprecated execCommand
copyKeyBtn.addEventListener("click", async () => {
  try {
    ...
    // Manual fallback - select text for user to copy
    streamKey.select();
    alert(
